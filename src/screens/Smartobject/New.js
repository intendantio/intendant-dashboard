...
class NewSmartobject extends React.Component {
    ...
    getSubmitButton() {
        if (this.state.reference.length == 0 || this.missingSettings()) {
            ...
            return (
                <a href={this.state.package.submit.url + (this.state.package.submit.url.includes("?") ? "&" : "?")+  "smartobject_id=false&reference=" + this.state.reference + this.getOauthSettings() + "&room=" + this.state.idRoom + "&redirect_uri=" + window.location.origin + "/smartobject/oauth/" + this.props.match.params.idSmartobject}>
